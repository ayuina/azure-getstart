...
![Windows仮想マシンの構成](./image/configure-web1.png)
...
![Linux 仮想マシンの構成](./image/create-config-web2.png)
...
![アプリ稼働確認](./image/application-error.png)
...
![テーブルの作成とデータ投入](./image/create-and-insert-table.png)
...
ロードバランサーはバックエンドプールに所属する仮想マシンのうち、稼働中の仮想マシンにのみリクエストを振り分ける必要があります。
ロードバランサーからは定期的なポーリングを行って正常なリクエストを返す仮想マシンを稼働中とみなします。
この処理を正常性プローブと呼びます。

![正常性プローブの定義](./image/configure-lb-prove.png)

Web サーバー側に監視用ポートや正常性確認用のカスタム Web ページを用意していませんので、ここでは「TCP ポート 80 と通信ができる」ことを Web サーバーが正常とみなすように構成しています。
...
ロードバランサーの各種構成の完了後、ロードバランサーと一緒に作成したパブリック IP アドレスを Web ブラウザーに入力すると、Web サーバー 1 号機ないしは 2 号機でホストされている Web ページが表示されます。
しかし Web ページは FQDN（Full Query Domain Name）でアクセスするのが一般的ですので、ここではパブリック IP アドレスにドメイン名のラベルを設定します。
